...
type AttributeQuery struct {
	...
	withUser         *UserQuery
	withOptions      *AttributeOptionQuery
	withOptionValues *AttributeOptionValueQuery
...
func (aq *AttributeQuery) QueryUser() *UserQuery {
...
func (aq *AttributeQuery) QueryOptions() *AttributeOptionQuery {
...
func (aq *AttributeQuery) QueryOptionValues() *AttributeOptionValueQuery {
	query := (&AttributeOptionValueClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AttributeQuery) Clone() *AttributeQuery {
	...
	return &AttributeQuery{
		...
		withOptions:      aq.withOptions.Clone(),
		withOptionValues: aq.withOptionValues.Clone(),
...
func (aq *AttributeQuery) WithUser(opts ...func(*UserQuery)) *AttributeQuery {
	query := (&UserClient{config: aq.config}).Query()
...
func (aq *AttributeQuery) WithOptions(opts ...func(*AttributeOptionQuery)) *AttributeQuery {
	query := (&AttributeOptionClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withOptions = query
	return aq
}
...
func (aq *AttributeQuery) WithOptionValues(opts ...func(*AttributeOptionValueQuery)) *AttributeQuery {
	query := (&AttributeOptionValueClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withOptionValues = query
	return aq
}
...
func (aq *AttributeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Attribute, error) {
	...
	if query := aq.withUser; query != nil {
		if err := aq.loadUser(ctx, query, nodes, nil,
			func(n *Attribute, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withOptions; query != nil {
		if err := aq.loadOptions(ctx, query, nodes,
			func(n *Attribute) { n.Edges.Options = []*AttributeOption{} },
			func(n *Attribute, e *AttributeOption) { n.Edges.Options = append(n.Edges.Options, e) }); err != nil {
			return nil, err
		}
	}
	if query := aq.withOptionValues; query != nil {
		if err := aq.loadOptionValues(ctx, query, nodes,
			func(n *Attribute) { n.Edges.OptionValues = []*AttributeOptionValue{} },
			func(n *Attribute, e *AttributeOptionValue) { n.Edges.OptionValues = append(n.Edges.OptionValues, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AttributeQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Attribute, init func(*Attribute), assign func(*Attribute, *User)) error {
...
func (aq *AttributeQuery) loadOptions(ctx context.Context, query *AttributeOptionQuery, nodes []*Attribute, init func(*Attribute), assign func(*Attribute, *AttributeOption)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(attributeoption.FieldAttributeID)
	}
	query.Where(predicate.AttributeOption(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(attribute.OptionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (aq *AttributeQuery) loadOptionValues(ctx context.Context, query *AttributeOptionValueQuery, nodes []*Attribute, init func(*Attribute), assign func(*Attribute, *AttributeOptionValue)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Attribute)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(attributeoptionvalue.FieldAttributeID)
	}
	query.Where(predicate.AttributeOptionValue(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(attribute.OptionValuesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (aq *AttributeQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := aq.ctx.Fields; len(fields) > 0 {
		...
		if aq.withUser != nil {
			_spec.Node.AddColumnOnce(attribute.FieldUserID)
		}
	}
