...
func (sq *StaffQuery) QueryUser() *UserQuery {
...
func (sq *StaffQuery) WithUser(opts ...func(*UserQuery)) *StaffQuery {
	query := (&UserClient{config: sq.config}).Query()
...
//
//	client.Staff.Query().
//		GroupBy(staff.FieldDepartmentID).
...
//
//	client.Staff.Query().
//		Select(staff.FieldDepartmentID).
//		Scan(ctx, &v)
...
func (sq *StaffQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Staff, error) {
	...
	if query := sq.withUser; query != nil {
		if err := sq.loadUser(ctx, query, nodes, nil,
			func(n *Staff, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *StaffQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Staff, init func(*Staff), assign func(*Staff, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Staff)
	for i := range nodes {
		if nodes[i].user_staff == nil {
			continue
		}
		fk := *nodes[i].user_staff
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_staff" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
