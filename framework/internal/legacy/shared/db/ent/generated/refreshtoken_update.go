...
func (rtu *RefreshTokenUpdate) check() error {
	if v, ok := rtu.mutation.Token(); ok {
		if err := refreshtoken.TokenValidator(v); err != nil {
			return &ValidationError{Name: "token", err: fmt.Errorf(`generated: validator failed for field "RefreshToken.token": %w`, err)}
		}
	}
	if rtu.mutation.UserCleared() && len(rtu.mutation.UserIDs()) > 0 {
		return errors.New(`generated: clearing a required unique edge "RefreshToken.user"`)
	}
	return nil
}
