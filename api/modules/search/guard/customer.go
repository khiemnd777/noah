...
func init() {
	logger.Debug("[GuardSearch] Register Customer")
	search.RegisterGuard("customer", func(ctx search.GuardCtx, rows []model.Row) []model.Row {
		perms := ctx.Perms

		if !rbac.HasAnyPerm(perms, "customer.search") {
			return []model.Row{}
		}
