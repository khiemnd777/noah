...
type DepartmentQuery struct {
	...
	withMembers  *DepartmentMemberQuery
...
func (dq *DepartmentQuery) QueryMembers() *DepartmentMemberQuery {
	query := (&DepartmentMemberClient{config: dq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := dq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (dq *DepartmentQuery) WithMembers(opts ...func(*DepartmentMemberQuery)) *DepartmentQuery {
	query := (&DepartmentMemberClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (dq *DepartmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Department, error) {
	...
	if query := dq.withMembers; query != nil {
		if err := dq.loadMembers(ctx, query, nodes,
			func(n *Department) { n.Edges.Members = []*DepartmentMember{} },
			func(n *Department, e *DepartmentMember) { n.Edges.Members = append(n.Edges.Members, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DepartmentQuery) loadMembers(ctx context.Context, query *DepartmentMemberQuery, nodes []*Department, init func(*Department), assign func(*Department, *DepartmentMember)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Department)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(departmentmember.FieldDepartmentID)
	}
