...
func (pq *PhotoQuery) QueryFolder() *FolderQuery {
	query := (&FolderClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PhotoQuery) Clone() *PhotoQuery {
	...
	return &PhotoQuery{
		...
		withFolder: pq.withFolder.Clone(),
...
func (pq *PhotoQuery) WithFolder(opts ...func(*FolderQuery)) *PhotoQuery {
	query := (&FolderClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PhotoQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Photo, error) {
	...
	if query := pq.withFolder; query != nil {
		if err := pq.loadFolder(ctx, query, nodes, nil,
			func(n *Photo, e *Folder) { n.Edges.Folder = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PhotoQuery) loadFolder(ctx context.Context, query *FolderQuery, nodes []*Photo, init func(*Photo), assign func(*Photo, *Folder)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Photo)
	for i := range nodes {
...
func (pq *PhotoQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := pq.ctx.Fields; len(fields) > 0 {
		...
		if pq.withFolder != nil {
			_spec.Node.AddColumnOnce(photo.FieldFolderID)
		}
	}
